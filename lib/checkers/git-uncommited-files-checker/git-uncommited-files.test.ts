...
import { gitUncommitedFilesChecker } from './index';
...
beforeEach(() => {
  ...
  gitUncommitedFilesChecker.errors = undefined;
  gitUncommitedFilesChecker.warnings = undefined;
  gitUncommitedFilesChecker.hasErrors = undefined;
  gitUncommitedFilesChecker.hasWarnings = undefined;
});
afterEach(() => {
...
test('It should not run when git is not installed', () => {
  // Given
  const checker = gitUncommitedFilesChecker;
...
test('It should run without error when there is no uncommitted file', () => {
  // Given
  const checker = gitUncommitedFilesChecker;
...
test('It should run without error when there are only untracked files', () => {
  // Given
  const checker = gitUncommitedFilesChecker;
...
test('It should run with error when there are uncommited files', () => {
  // Given
  const checker = gitUncommitedFilesChecker;
  ...
  const error1: ValidationError = { reason: `File 'package.json' is uncommited`, severity: 'error' };
